    ...
    public class EventsController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<ScheduleEventResponse>>> Get()
        {
            return await _db.ScheduleEvents
                .Where(x => x.User.Login == User.Identity.Name)
        ...
        public async Task<ActionResult<ScheduleEventResponse>> Post(ScheduleEvent scheduleEvent)
        {
            ...
            var user = await _db.Users
                .SingleOrDefaultAsync(x => x.Login == User.Identity.Name);
            if(user == null)
        ...
        public async Task<ActionResult<ScheduleEventResponse>> Put(ScheduleEvent scheduleEvent)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            var dbEvent = await _db.ScheduleEvents
                .SingleOrDefaultAsync(x => x.Id == scheduleEvent.Id);
            if (dbEvent == null)
                return NotFound();
            var user = await _db.Users
                .SingleOrDefaultAsync(x => x.Login == User.Identity.Name);
        ...
        {
            var scheduleEvent = _db.ScheduleEvents
                .Include(x => x.User)
